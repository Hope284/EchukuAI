...
class GoogleCalendarOAuthController extends Controller
{
    ...
    {
        abort_unless(ConnectorPlanGate::allows(Auth::user(), self::CONNECTOR_TYPE), 404);
        ...
        try {
            ...
            Log::error('[DZEVA Connector] Calendar authorization failed to start.', ['error' => $exception->getMessage()]);

            return back()->with(['type' => 'error', 'message' => __('This connector is temporarily unavailable. Please contact an administrator.')]);
        }
    }
    ...
    public function callback(Request $request): RedirectResponse|View
    {
        ...
        try {
            ...
            Log::warning('[DZEVA Connector] Calendar authorization callback failed.', ['error' => $exception->getMessage()]);

            return $isPopup
                ? $this->popupClose(false, 'This connector is temporarily unavailable. Please try again.', null)
                : $this->redirectWith('error', 'This connector is temporarily unavailable. Please try again.');
        }
