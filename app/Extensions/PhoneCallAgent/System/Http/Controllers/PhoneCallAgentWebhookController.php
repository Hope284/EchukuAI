...
XML;
    ...
    {
        $expectedSignature = hash_hmac('sha256', $agentUuid . ':' . $toolName, (string) config('app.key'));
        ...
        Log::info('[Phone Call Agent] Booking tool invoked', ['tool' => $toolName, 'agent_id' => $agent->id]);
        ...
        try {
            ...
            $result = 'Booking action failed. Please try again.';
        }
    ...
    {
        $secret = config('phone-call-agent.elevenlabs_webhook_secret');

        if (empty($secret)) {
            Log::error('PhoneCallAgent: ElevenLabs webhook secret is not configured.');
        ...
        if (abs(now()->timestamp - (int) $timestamp) > 300) {
            Log::warning('PhoneCallAgent: stale ElevenLabs webhook signature.');
