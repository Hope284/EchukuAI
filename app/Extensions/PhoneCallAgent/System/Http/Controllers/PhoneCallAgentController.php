...
class PhoneCallAgentController extends Controller
{
    ...
    public function store(PhoneCallAgentStoreRequest $request): JsonResource|JsonResponse
    {
        ...
        report(new RuntimeException((string) ($resData->message ?? $resData->error ?? 'Phone agent creation failed.')));

        return response()->json([
            'message' => __('Phone agent setup is temporarily unavailable. Please try again later.'),
        ], 422);
    }
    ...
    public function simulate(ExtPhoneCallAgent $agent): JsonResponse
    {
        ...
        if ($agent->provider->value !== 'elevenlabs' || ! $agent->agent_id) {
            return response()->json([
                'message' => __('Web simulation is not available for this call connection type.'),
            ], 422);
        }
