...
class LinkParser
{
    ...
    private function crawlPage(string $url, bool $single): void
    {
        ...
        if (isset($this->contents[$url])) {
            return;
        }

        $html = SafeRemoteUrl::fetchText($url);
        if ($html === null) {
            return;
        }
    ...
    {
        if (! SafeRemoteUrl::isAllowed($baseUrl)) {
            throw new \InvalidArgumentException('Only public HTTP or HTTPS URLs can be crawled.');
        }
