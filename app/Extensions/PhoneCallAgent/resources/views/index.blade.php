            ...
            document.addEventListener('alpine:init', () => {
                Alpine.data('phoneCallAgentEditor', () => ({
                    ...
                    defaultFormInputs: {
                        ...
                        phone_number: '',
                        phone_number_id: '',
                        service_type: '{{ setting('phone_call_agent_provider', 'elevenlabs') === 'twilio' ? 'direct' : 'managed' }}',
                    ...
                    async setEditingStep(step, skipCRUD = false) {
                        ...
                        if (
                            editingStep === 3 &&
                            this.activeAgent.id !== 'new_agent' &&
                            this.activeAgent.service_type !== 'direct' &&
                    ...
                    async importPhoneNumber() {
                        ...
                        toastr.success(this.activeAgent.service_type === 'direct'
                    ...
                    async releasePhoneNumber() {
                        ...
                        if (!confirm('{{ __('Are you sure you want to remove this phone number from the DZEVA voice workspace?') }}')) {
                            return;
                        }
                    ...
                    async startSimulation() {
                        ...
                        if (this.activeAgent.service_type === 'direct') {
                            toastr.info('{{ __('Web simulation is not available for this call connection type.') }}');
                            return;
                        }
                        ...
                        try {
                            ...
                            if (!window.ElevenLabsConversation) {
                                await new Promise((resolve, reject) => {
                                    ...
                                    s.onerror = () => reject(new Error('{{ __('Failed to load the voice simulation service.') }}'));
                            ...
                            if (!Conversation) {
                                throw new Error('{{ __('Voice simulation is temporarily unavailable.') }}');
                            }
