...
>
    ...
    <div class="flex flex-col gap-7 pt-9">
		{{-- Public connection label; backend provider keys remain internal. --}}
        <div>
            <label class="form-label text-heading-foreground">@lang('Call Connection')</label>
            <div class="flex items-center gap-2 rounded-lg border bg-heading-foreground/[2%] px-4 py-3 text-sm capitalize">
                ...
                <span x-text="activeAgent.service_type === 'direct' ? '{{ __('Direct Call Network') }}' : '{{ __('DZEVA Voice Network') }}'"></span>
            </div>
        </div>

        {{-- ElevenLabs Agent ID (hidden for Twilio agents) --}}
        <template x-if="activeAgent.service_type !== 'direct'">
            <div>
                <label class="form-label text-heading-foreground">@lang('DZEVA Voice Agent ID')</label>
        ...
        <template x-if="(activeAgent.service_type === 'managed' && activeAgent.agent_id) || activeAgent.service_type === 'direct'">
            <div class="flex flex-col gap-5">
                ...
                <x-modal id="phone-import-modal">
                    <x-slot:title>
                        <span x-text="activeAgent.service_type === 'direct' ? '{{ __('Connect a Phone Number') }}' : '{{ __('Import a Phone Number') }}'"></span>
                    </x-slot:title>

                    <x-slot:trigger class="w-full" variant="outline" type="button">
                        <x-tabler-download class="size-4" />
                        <span x-text="activeAgent.service_type === 'direct' ? '{{ __('Connect Phone Number') }}' : '{{ __('Import Phone Number') }}'"></span>
                    </x-slot:trigger>
                    ...
                    <x-slot:modal>
                        ...
                        >
                            <p class="text-2xs opacity-60">
                                <span x-show="activeAgent.service_type === 'direct'">@lang('Connect your own phone number. DZEVA configures the secure call connection automatically. Credentials are stored on this agent only.')</span>
                                <span x-show="activeAgent.service_type !== 'direct'">@lang('Import a number into your DZEVA voice workspace and attach it to this agent.')</span>
                            </p>

                            {{-- Source selector (ElevenLabs only — Twilio agents always use their own Twilio number) --}}
                            <div x-show="activeAgent.service_type !== 'direct'">
                                <x-forms.input
                                ...
                                >
                                    <option value="twilio">@lang('Direct Call Account')</option>
                                    <option value="sip_trunk">@lang('SIP Trunk')</option>
                                    <option value="exotel">@lang('Regional Call Account')</option>
                                </x-forms.input>
                            </div>
                            ...
                            >
                                <x-tabler-download class="size-4" />
                                <span x-text="activeAgent.service_type === 'direct' ? '{{ __('Connect & Configure') }}' : '{{ __('Import & Attach') }}'"></span>
                            </x-button>
                        </div>
                    </x-slot:modal>
                </x-modal>
