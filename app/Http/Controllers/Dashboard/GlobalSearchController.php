    ...
    public function aiStream(Request $request): StreamedResponse
    {
        ...
        return response()->stream(function () use ($query, $model, $driver) {
            ...
            try {
                ...
                Log::error('[DZEVA Global Search] AI response failed.', ['error' => $e->getMessage()]);
                echo "event: data\n";
                echo 'data: ' . json_encode(['error' => __('AI search is temporarily unavailable. Please try again.')]) . "\n\n";
                flush();
            }
